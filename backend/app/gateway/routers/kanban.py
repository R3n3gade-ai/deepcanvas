...
async def _get_lock(workspace_id: str) -> asyncio.Lock:
    async with _global_lock:
        if workspace_id not in _board_locks:
            _board_locks[workspace_id] = asyncio.Lock()
        return _board_locks[workspace_id]
...
def _load_board(workspace_id: str) -> dict:
    ...
    if p.exists():
        ...
        # Guard against corrupted / duplicate JSON
        try:
            data = json.loads(raw)
        except json.JSONDecodeError:
            # Try to recover first valid object
            idx = raw.find("}{")
            if idx > 0:
                try:
...
async def update_board(workspace_id: str, board: dict):
    ...
    async with (await _get_lock(workspace_id)):
